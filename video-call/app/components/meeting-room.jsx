'use client';
import { Call, CallControls, CallRecordingList, SpeakerLayout, StreamCall, useStreamVideoClient } from '@stream-io/video-react-sdk';
...
const MeetingRoom = ({callId,onLeave,userId}) => {
    ...
    if(error)
        return (
            <div className='flex items-center justify-center min-h-screen text-white'>Error: {error}</div>
        );
    if(!call)
        return(
            <div className='flex items-center justify-center min-h-screen text-white'>
                <div className='animate-spin h-16 w-16 border-t-4 border-blue-500 rounded-full'><p className='mt-4 text-lg'>Loading Meeting...</p></div>
            </div>
        );
        ...
        <div className='min-h-screen bg-linear-to-br from-gray-900 via-gray-800 to-gray-900 text-white container mx-auto px-4 py-6'>
            <div className='grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-6 h-screen'>
                <div className='flex flex-col gap-4'>
                    ...
                    {/* Transcription */}
                <div className='bg-gray-800 rounded-2xl border border-gray-700 overflow-hidden shadow-2xl'><TranscriptPanel /></div>
            </div>
        </div>
        </StreamCall>
    );
