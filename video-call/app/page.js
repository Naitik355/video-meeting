...
export default function Home() {
  ...
  const handleJoin=()=>{
    ...
    router.push(`/meeting/${meetingId}?username=${encodeURIComponent(name)}`);
  };
  return (
    <div className="flex items-center justify-center min-h-screen bg-linear-to-br from-gray-900 via-gray-800 to-gray-900 text-white">
      <div className="p-8 bg-gray-800 rounded-2xl border border-gray-700 w-80 shadow-2xl">
        ...
        <input placeholder="e.g. Naitik" className="px-4 py-3 w-full rounded-lg bg-gray-700/80 border border-gray text-white" value={username} onChange={(e)=>setUsername(e.target.value)}></input>
        <button onClick={handleJoin} className="mt-5 w-full py-3 bg-blue-600 hover:bg-blue-700 rounded-lg font-medium">Join Meeting</button>
      </div>
    </div>
  );
}
